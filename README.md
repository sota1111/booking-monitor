...
予約サイトの空き状況を監視し、空きが見つかった場合に Discord で通知するシステムです。リサーチ（予約サイト確認）はローカルから実行し、Web（Cloud Run）は Cloud Firestore に保存されたデータの表示と、入力された監視対象の Firestore への登録のみを担当します。

> **構成の最適化（SOT-1300）**: Web は「表示 + Firestore 登録」専用です。Web からリサーチを起動する `POST /run` と手動実行ボタンは廃止しました。リサーチは `python main.py`（ローカル）から実行し、結果は Firestore に保存されて Web に表示されます。本番（Cloud Run）の Cloud Scheduler 経由の自動監視は停止しています。
...
ローカル実行（cron 等で定期実行）
  python main.py ──► run_scheduler
                     ├──► 監視対象を Firestore（monitoring_targets）から取得
                     ├──► 予約サイト確認 (Playwright / HTTPX)
                     ├──► Firestore（GOOGLE_CLOUD_PROJECT 設定時）または ローカル JSONL に保存
                     │       ├── monitoring_results / history.jsonl（最新状態）
                     │       ├── check_history（時系列履歴）
                     │       └── notification_history（通知履歴）
                     └──► Discord Webhook（状態変化時のみ）

ブラウザ (ログイン後) ─ Web は表示 + 登録のみ
  └──GET /──────► カレンダー概観（範囲監視の日付×時刻スロット）
  └──GET /monitor──► 監視対象一覧 + 監視対象の登録（POST /targets → Firestore）
  └──GET /notification-history──► 通知履歴・監視履歴
  └──GET /config──► 設定確認
```
...
リサーチ（予約サイトの空き確認）は **ローカルから** 実行します（SOT-1300）。Web に `POST /run` のような実行エンドポイントはありません。

```bash
python main.py
```

- 監視対象は Firestore（`GOOGLE_CLOUD_PROJECT` 設定時、コレクション `monitoring_targets`）から取得します。未設定時は `config.json` の `targets` を使用します。
- チェック結果・通知履歴は `get_history()` 経由で保存されます（`GOOGLE_CLOUD_PROJECT` 設定時は Firestore、未設定時はローカル `logs/*.jsonl`）。Firestore に保存された結果は Web で表示されます。
- 定期実行したい場合は cron などで `python main.py` をスケジュールしてください（本番 Cloud Run 側の自動監視は停止しています）。
...
| パス | 画面名 | 説明 |
|------|--------|------|
| `/` | カレンダー概観 | 範囲監視（日付 × 時刻）の各スロットの空き/満席/不明を、カレンダー形式で概観表示します。 |
| `/monitor` | 監視対象 | 監視対象一覧の表示と、監視対象の登録（フォーム送信で Firestore に登録）。 |
| `/notification-history` | 通知履歴・監視履歴 | Discord 通知の送信日時・対象店舗・成功/失敗/スキップと、チェックごとの監視履歴を表示。Webhook URL などの秘密情報は表示されません。 |
| `/config` | 設定確認 | 監視条件（キーワード・チェック間隔・人数条件など）を画面で確認できます。秘密情報は表示されません。 |

> リサーチ（チェック実行）は Web ではなくローカルの `python main.py` から行います（SOT-1300）。手動実行ボタンや `POST /run` は廃止されました。
...
- 状態バッジ:
  ...
  - 🔵 **未確認** — まだチェックが実行されていない
  - 🔴 **取得失敗** — チェック中にエラーが発生
- 範囲監視（日付 × 時刻）を使う対象がある場合、サマリーに **空きスロット** と **監視スロット計** を追加表示します。
- リサーチ（チェック実行）はローカルの `python main.py` で行います。Web からの手動実行は廃止されました（SOT-1300）。

### サンプルデータでダッシュボードを評価する（`SEED_SAMPLE_DATA`）

実スクレイピングなしでダッシュボードの表示を評価するため、サンプル（仮）データを登録できます（SOT-1152）。

- **CLI で生成**:

  ```bash
  python scripts/seed_sample_data.py          # config.sample.json + logs/*.jsonl を生成
  python scripts/seed_sample_data.py --force  # サンプル config も再生成
  ```

  サンプルの監視対象（範囲監視を含む 5 件）と、最新状態 / 監視履歴 / 通知履歴の各 JSONL を生成します。

- **起動時に自動生成**: 環境変数 `SEED_SAMPLE_DATA=1`（`1|true|yes|on`）を設定して Web を起動すると、
  起動時にサンプル config（`config.sample.json`）と履歴を生成し、ダッシュボードがサンプル対象を表示します。
  画面上部に「サンプルデータ表示中」バナーが出ます。**本番では未設定のまま**にしてください（既定 OFF＝無効）。
...
Web（Cloud Run / Docker）は表示と登録のみを行います。リサーチの動作確認はローカルで以下を実行します。
```bash
python main.py
```
...
```bash
# 例: 1時間ごとにローカルでリサーチを実行（crontab -e）
0 * * * * cd /path/to/booking-monitor && /path/to/.venv/bin/python main.py >> logs/cron.log 2>&1
```

監視対象を Firestore と共有し、結果を Web に表示させるには、ローカル側でも `GOOGLE_CLOUD_PROJECT`（必要に応じて `FIRESTORE_TARGETS_COLLECTION` / `FIRESTORE_DATABASE_ID`）を設定して同じ Firestore に接続してください。
...
| サービス | 無料枠 | 想定使用量 |
|---------|-------|---------|
| Cloud Run | 180,000 vCPU 秒/月、360,000 GiB 秒/月 | 表示・登録のみで軽量 ✓ |
| Firestore | 50,000 読み取り/日、20,000 書き込み/日、1GiB | 1 回の実行で数読み書き ✓ |
| Container Registry | 0.5GiB/月まで無料 | 定期的なクリーンアップを推奨 |

リサーチ（チェック実行）はローカルで動くため Cloud Run / Cloud Scheduler の費用はかかりません（SOT-1300）。複数の監視対象は `monitoring_targets` コレクション（Firestore）または `config.json` の `targets` にまとめて定義できます。
